...
function EditService() {
  ...
  // Fetch initial data (service + masters)
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        const [serviceData, portfolioData, sectorData, unitData] = await Promise.all([
          serviceRes.json(),
          portfolioRes.json(),
          sectorRes.json(),
          unitRes.json(),
        ...
        setForm({
          ...
          portfolio_id: s.portfolio?.id?.toString() || '',
          sub_portfolio_id: s.sub_portfolio?.id?.toString() || '',
          sbu_owner_id: s.sbu_owner?.id?.toString() || '',
          sectors: s.sectors?.map((sec) => sec.id.toString()) || [],
    ...
    if (authToken) fetchAll();
  }, [authToken, id, toast]);

  // Cascade sub-sectors when sectors change; also prune invalid sub_sectors
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // sectors must be parents of selected sub_sectors
      const filteredSectors = sectors
        .filter((s) =>
          s.sub_sectors?.some((sub) => form.sub_sectors.includes(sub.id.toString()))
        )
        .map((s) => s.id.toString());
      ...
      if (!res.ok) throw new Error(data.error || 'Gagal memperbarui layanan');
  ...
  // Create master data (same as TambahJasa)
  const handleCreatePortfolio = async () => {
    ...
    try {
      ...
      setForm((prev) => ({ ...prev, portfolio_id: data.portfolio.id.toString(), sub_portfolio_id: '' }));
  ...
  const handleCreateSubPortfolio = async () => {
    ...
    try {
      ...
      setForm((prev) => ({ ...prev, sub_portfolio_id: data.sub_portfolio.id.toString() }));
  ...
  return (
    <>
      <Helmet>
        <title>Edit Layanan - SAKTI</title>
      </Helmet>
      ...
      >
        {/* HEADER / HERO (selaras dengan TambahJasa) */}
          ...
          <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-blue-100 text-sm sm:text-base">
                Perbarui informasi layanan agar tetap akurat dan rapi di SAKTI.
              </p>
            </div>
            <Link to="/daftar-jasa">
          ...
          <Card className="shadow-md border border-gray-100 rounded-2xl hover:shadow-lg transition duration-300">
            ...
            <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-6">
              <div>
                ...
                <Input name="name" value={form.name} onChange={handleChange} className="mt-2" required />
              </div>
                              ...
                              <button
                                ...
                                onClick={(e) => {
                                  ...
                                  setForm((prev) => ({
                                    ...
                                    sub_sectors: prev.sub_sectors.filter(
                                      (ssId) => !sektor.sub_sectors?.some((ss) => ss.id.toString() === ssId)
                                    ),
                                  }));
                                }}
