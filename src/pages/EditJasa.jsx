/**
 * pages/EditService.jsx
 *
 * Halaman untuk mengedit layanan (service) yang sudah ada.
 * Menampilkan form lengkap dengan field: informasi dasar, portfolio & sub portfolio,
 * sektor & sub sektor, manfaat, output, ruang lingkup, video pengantar, regulasi.
 *
 * ============================================================
 * FITUR UTAMA
 * ============================================================
 * - Mengambil data layanan berdasarkan ID dari URL
 ...
 * - Validasi sisi client untuk field wajib
 * - Preview video YouTube dari URL yang dimasukkan
 * - Penyimpanan melalui PUT /services/:id
 * - Redirect ke halaman detail setelah sukses
 *
 * ============================================================
 * STRUKTUR DATA API
 * ============================================================
 * GET    /services/:id                     → ambil data layanan
 ...
 * - Form submisi mengubah daftar sektor terpilih menjadi sektor yang memiliki
 *   setidaknya satu sub sektor yang dipilih (sesuai kebutuhan backend).
 * - Semua modal untuk membuat master data menggunakan state terpisah.
 */
...
import {
  ArrowLeft, Save, Building, Star, FileText,
  LayoutDashboard, ScrollText, Info, Package, Globe,
  CheckCircle2, Video, Youtube, FolderKanban,
  Sparkles, AlertCircle, Hash, Check, Search,
  X, Plus, ChevronRight
} from 'lucide-react';
...
/**
 * Hero card (header) menampilkan nama layanan yang sedang diedit.
 */
const HeroCard = ({ name }) => (
  <motion.div
    initial={{ opacity: 0, y: 16 }}
...
/**
 * Loading skeleton saat mengambil data awal.
 */
const LoadingSkeleton = () => (
  <div className="flex justify-center items-center min-h-[60vh]">
    <div className="text-center">
      <div className="w-12 h-12 border-4 border-[#000476] border-t-transparent rounded-full animate-spin mx-auto mb-4" />
...
/**
 * EditService - Halaman untuk mengedit layanan yang sudah ada.
 * @returns {JSX.Element}
 */
export default function EditService() {
  ...
  const canCreateMasterData = user?.role === ROLES.ADMIN || user?.role === ROLES.MANAGEMENT;
  ...
  const [subSectors, setSubSectors] = useState([]);    // sub sektor yang tersedia berdasarkan sektor terpilih
  const [sbuUnits, setSbuUnits] = useState([]);        // unit dengan tipe 'sbu'
  ...
  // State untuk Popover sektor
  const [openSector, setOpenSector] = useState(false);

  // ============================================================
  // useEffect: Ambil data layanan + data master
  // ============================================================
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        const serviceData = serviceRes.data?.data || serviceRes.data?.service || serviceRes.data;
        ...
        const unitsData = unitRes.data?.data || unitRes.data?.units || [];

        // Filter unit yang bertipe 'sbu'
        const sbuList = unitsData.filter(unit => unit.type === 'sbu');
        ...
        setForm({
          ...
          intro_video_url: serviceData.intro_video_url || '',
          overview: serviceData.overview || '',
          scope: serviceData.scope || '',
          benefit: serviceData.benefit || '',
          output: serviceData.output || '',
          regulation_ref: serviceData.regulation_ref || '',
          portfolio_id: serviceData.portfolio?.id?.toString() || '',
          sub_portfolio_id: serviceData.sub_portfolio?.id?.toString() || '',
          sbu_owner_id: serviceData.sbu_owner?.id?.toString() || '',
          sectors: serviceData.sectors?.map(sec => sec.id.toString()) || [],
          sub_sectors: serviceData.sub_sectors?.map(ss => ss.id.toString()) || [],
        });
  ...
  useEffect(() => {
    ...
    // Hapus sub sektor yang tidak lagi valid (milik sektor yang tidak dipilih)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // (atau jika tidak ada sub sektor, tetap kirim sektor yang dipilih)
  ...
  const handleCreateSubPortfolio = async () => {
    ...
    try {
      ...
      // Update portfolio list agar sub portfolio baru muncul di dropdown
      setPortfolios(prev =>
        prev.map(p =>
  ...
  const handleCreateSubSector = async () => {
    ...
    try {
      ...
      // Update sector list agar sub sektor baru muncul di daftar
      setSectors(prev =>
        prev.map(s =>
      ...
      // Tambahkan sub sektor ke form jika sektor induk sedang dipilih
      setForm(prev => ({
        ...prev,
  ...
  return (
    <>
      ...
      <div className="bg-gray-50 min-h-screen">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          ...
          <div className="flex justify-between items-center mb-3">
            <Link to={`/detail-jasa/${id}`}>
              <Button variant="outline" className="shadow-sm">
                <ArrowLeft className="w-4 h-4 mr-2" /> Kembali ke Detail
              </Button>
            </Link>
            <Button
                          ...
                          >
                            {form.sectors.length > 0 ? (
                              form.sectors.map(id => {
                                ...
                                return s && (
                                  <Badge key={s.id} variant="secondary" className="bg-indigo-50 text-indigo-800">
      ...
      <Dialog open={showPortfolioModal} onOpenChange={setShowPortfolioModal}>
        <DialogContent>
          <DialogHeader><DialogTitle>Tambah Portfolio Baru</DialogTitle></DialogHeader>
          <Input
            value={newPortfolioName}
            onChange={e => setNewPortfolioName(e.target.value)}
            placeholder="Nama Portfolio"
          />
          <Button onClick={handleCreatePortfolio} className="mt-3">Simpan</Button>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showSubPortfolioModal} onOpenChange={setShowSubPortfolioModal}>
        <DialogContent>
          <DialogHeader><DialogTitle>Tambah Sub Portfolio Baru</DialogTitle></DialogHeader>
      ...
      <Dialog open={showSectorModal} onOpenChange={setShowSectorModal}>
        <DialogContent>
          <DialogHeader><DialogTitle>Tambah Sektor Baru</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <Input
      ...
      <Dialog open={showSubSectorModal} onOpenChange={setShowSubSectorModal}>
        <DialogContent>
          <DialogHeader><DialogTitle>Tambah Sub Sektor Baru</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <select
